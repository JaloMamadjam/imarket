...
export async function menuFornecedor() {
    let sair = false;
    while (!sair) {
        clearScreen();
        const opcao = await select({
            message: 'Gerenciamento de Fornecedores:',
            choices: [
        ...
        try {
            switch (opcao) {
                ...
                case '2':
                    ...
                    break;
                case '3':
                    const nome = await input({ message: 'Nome da empresa:' });
                    const email = await input({ message: 'E-mail:' });
                    const telefone = await input({ message: 'Telefone:' });
                    await apiClient.post(ENDPOINT, { nome, email, telefone });
                ...
                case '4':
                    ...
                    await apiClient.put(`${ENDPOINT}/${idAtualizar}`, { nome: nNome, email: nEmail, telefone: nTel });
