...
export async function menuCliente() {
    ...
    while (!sair) {
        ...
        const opcao = await select({
            message: 'Gerenciamento de Clientes:',
            choices: [
        ...
        try {
            switch (opcao) {
                ...
                case '2':
                    ...
                    break;
                case '3':
                    const cpf = await input({ message: 'CPF:' });
                    ...
                    await apiClient.post(ENDPOINT, { cpf, nome, email, telefone });
                ...
                case '4':
                    ...
                    const nCpf = await input({ message: 'CPF:', default: atual.cpf });
                    const nNome = await input({ message: 'Nome:', default: atual.nome });
                    const nEmail = await input({ message: 'E-mail:', default: atual.email });
                    const nTel = await input({ message: 'Telefone:', default: atual.telefone });

                    await apiClient.put(`${ENDPOINT}/${idAtualizar}`, { cpf: nCpf, nome: nNome, email: nEmail, telefone: nTel });
