...
export async function menuInstituicao() {
    let sair = false;
    while (!sair) {
        clearScreen();
        const opcao = await select({
            message: 'Gerenciamento de Instituições:',
            choices: [
        ...
        try {
            switch (opcao) {
                ...
                case '2':
                    ...
                    break;
                case '3':
                    const nome = await input({ message: 'Nome da instituição:' });
                    const endereco = await input({ message: 'Endereço:' });
                    const telefone = await input({ message: 'Telefone:' });
                    await apiClient.post(ENDPOINT, { nome, endereco, telefone });
                ...
                case '4':
                    ...
                    const nEnd = await input({ message: 'Endereço:', default: atual.endereco });
                    const nTel = await input({ message: 'Telefone:', default: atual.telefone });

                    await apiClient.put(`${ENDPOINT}/${idAtualizar}`, { nome: nNome, endereco: nEnd, telefone: nTel });
