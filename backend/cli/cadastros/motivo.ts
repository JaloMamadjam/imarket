...
const ENDPOINT = '/motivos';

export async function menuMotivo() {
    let sair = false;
    while (!sair) {
        clearScreen();
        const opcao = await select({
            message: 'Gerenciamento de Motivos de Desperdício:',
            choices: [
        ...
        try {
            switch (opcao) {
                ...
                case '2':
                    ...
                    break;
                case '3':
                    const descricao = await input({ message: 'Descrição do motivo:' });
                    await apiClient.post(ENDPOINT, { descricao });
                ...
                case '4':
                    ...
                    const nDesc = await input({ message: 'Descrição:', default: atual.descricao });

                    await apiClient.put(`${ENDPOINT}/${idAtualizar}`, { descricao: nDesc });
